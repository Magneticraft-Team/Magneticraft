...
val ELECTRIC_POLE_PLACE = PropertyEnum.create("place", ElectricPoleTypes::class.java)

object BlockElectricPole : BlockState(Material.WOOD, "electric_pole"), ITileEntityProvider {
    ...
    override fun onBlockPlacedBy(worldIn: World, pos: BlockPos, state: IBlockState, placer: EntityLivingBase, stack: ItemStack?) {
        ...
        worldIn.setBlockState(pos, defaultState.withProperty(ELECTRIC_POLE_PLACE, ElectricPoleTypes.DOWN_3))
        worldIn.setBlockState(pos.offset(EnumFacing.UP, 1), defaultState.withProperty(ELECTRIC_POLE_PLACE, ElectricPoleTypes.DOWN_2))
        worldIn.setBlockState(pos.offset(EnumFacing.UP, 2), defaultState.withProperty(ELECTRIC_POLE_PLACE, ElectricPoleTypes.DOWN_1))
        worldIn.setBlockState(pos.offset(EnumFacing.UP, 3), defaultState.withProperty(ELECTRIC_POLE_PLACE, dir))
    }

    override fun breakBlock(worldIn: World, pos: BlockPos, state: IBlockState) {
        super.breakBlock(worldIn, pos, state)
        val place = ELECTRIC_POLE_PLACE[state]
        if (place == ElectricPoleTypes.DOWN_1 || place == ElectricPoleTypes.DOWN_2 || place == ElectricPoleTypes.DOWN_3) {

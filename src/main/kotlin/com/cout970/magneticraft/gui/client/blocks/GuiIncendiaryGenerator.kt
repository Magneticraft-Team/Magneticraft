...
class GuiIncendiaryGenerator(container: ContainerBase) : GuiBase(container) {
    ...
    override fun initComponents() {
        ...
        components.add(CompElectricBar(tile.mainNode, Vec2d(69, 56) + box.start))
        ...
        components.add(CompVerticalBar(
                ...
                3, Vec2d(102, 56) + box.start))

        components.add(CompGreenLight(Vec2d(68, 58) + box.start, { tile.mainNode.voltage < 120 }))
        components.add(CompGreenLight(Vec2d(79, 58) + box.start, { tile.burningTime > 0 }))
        components.add(CompGreenLight(Vec2d(90, 58) + box.start, { tile.heat >= STANDARD_AMBIENT_TEMPERATURE + 5 }))
        components.add(CompGreenLight(Vec2d(101, 58) + box.start, { tile.production.storage > 0 }))
    }
}
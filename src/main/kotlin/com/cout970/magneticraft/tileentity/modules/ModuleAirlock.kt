...
class ModuleAirlock(
    ...
    fun buildBubbles() {
        val bubble = ElectricMachines.airBubble
            ...
            for (k in -range..range) {
                for (i in -range..range) {
                    if (i * i + j * j + k * k <= length) {
                        ...
                        if (array[x][y][z] == bubble) {
                            if (node.voltage >= ElectricConstants.TIER_1_MACHINES_MIN_VOLTAGE) {
                                world.setBlockState(pos.add(i, j, k), ElectricMachines.DecayMode.OFF.getBlockState(bubble))
    ...
    fun startBubbleDecay(world: World, pos: BlockPos, range: Int, length: Int) {
        val bubble = ElectricMachines.airBubble
        val stateOn = ElectricMachines.DecayMode.ON.getBlockState(bubble)
            ...
            for (k in -range..range) {
                for (i in -range..range) {
                    if (i * i + j * j + k * k <= length) {
                        ...
                        if (state.block == ElectricMachines.airBubble) {
                            if (state[ElectricMachines.PROPERTY_DECAY_MODE] == ElectricMachines.DecayMode.OFF) {

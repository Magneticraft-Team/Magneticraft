...
open class OreGenerator(
    ...
    fun generate(world: World, random: Random, x: Int, y: Int, z: Int) {
        ...
        val posX = x + 8 + sin(angle.toDouble()) * config.veinAmount / 8.0f
        val negX = x + 8 - sin(angle.toDouble()) * config.veinAmount / 8.0f
        val posZ = z + 8 + cos(angle.toDouble()) * config.veinAmount / 8.0f
        val negZ = z + 8 - cos(angle.toDouble()) * config.veinAmount / 8.0f
        ...
        for (n in 0 until config.veinAmount) {
            ...
            val desp = (sin(n * Math.PI / config.veinAmount) + 1.0f) * scale + 1.0
